...
freecell :: [[Card]] -> Event MouseEvent -> Reactive (BehaviorTree [Sprite])
freecell stackCards eMouse = do
    ...
    return $ toTree stSprites :++ toTree ceSprites :++ BehaviorNode grSprites :++ BehaviorNode drSprites
  where
    toTree = foldr1 (:++) . map BehaviorNode
...
mkFreecell :: IO (Event MouseEvent -> Reactive (BehaviorTree [Sprite]))
mkFreecell = do
    rng <- newStdGen
    let (cards, rng') = shuffle rng [minBound..maxBound]
    return $ freecell (toStacks noOfStacks cards)
